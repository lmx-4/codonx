use anyhow::Context;
use ruff_python_ast::{Alias, Expr, Parameters, Stmt, StmtFunctionDef, Suite};
...
pub fn render_codon_candidate(input: &Path) -> anyhow::Result<String> {
    let source = fs::read_to_string(input)
        .with_context(|| format!("failed to read Python source {}", input.display()))?;
    let parsed = parse_module(&source)
        .map_err(|err| anyhow::anyhow!("Ruff parser failed for {}: {}", input.display(), err))?;
    let mut generator = CodonCandidateGenerator::new(&source);
    Ok(generator.render(parsed.suite()))
}
...
struct CodonCandidateGenerator<'a> {
    source: &'a str,
...
impl<'a> CodonCandidateGenerator<'a> {
    fn new(source: &'a str) -> Self {
        ...
        Self {
            ...
            line_starts,
            out: String::new(),
            codon_python: collect_codon_python_define(source),
        }
    }

    fn render(&mut self, suite: &Suite) -> String {
        self.out
            .push_str("# Generated by codonx py-codon. This is a conservative Codon candidate.\n");
        self.out.push_str(
            "# Default imports are routed through Codon's Python interop; native-marked imports keep Codon import semantics.\n",
        );
        if let Some(path) = &self.codon_python {
            self.out
                .push_str(&format!("# codonx: compile/run with CODON_PYTHON={path}\n"));
        }
        self.out.push('\n');
        self.render_suite(suite);
        self.out.clone()
    }

    fn render_suite(&mut self, suite: &Suite) {
        for stmt in suite {
        ...
        match stmt {
            Stmt::Import(import) => {
                if self.has_leading_codon_macro(stmt.range()) {
                    self.push_source_without_macro_lines(stmt.range());
                } else {
                    let indent = self.indent_for_range(stmt.range());
                    for alias in &import.names {
                        self.push_line(
                            indent,
                            &format!("from python import {}", render_alias(alias)),
                        );
                    }
                }
            }
            Stmt::ImportFrom(import_from) => {
                if self.has_leading_codon_macro(stmt.range()) {
                    ...
                    let Some(module) = import_from.module.as_ref() else {
                        self.push_line(
                            indent,
                            "# codonx: skipped relative import without an absolute module; Python fallback cannot be generated mechanically",
                        );
                        return;
                    };
                    let module = module.as_str();
                    let py_module = format!("__codonx_py_{}", sanitize_module_name(module));
                    self.push_line(
                        indent,
                        &format!("from python import {module} as {py_module}"),
                    );
                    for alias in &import_from.names {
                        ...
                        if name == "*" {
                            self.push_line(
                                indent,
                                "# codonx: wildcard import fallback is intentionally not expanded",
                            );
                            continue;
                        }
                        let binding = alias
                            ...
                            .unwrap_or(name);
                        self.push_line(indent, &format!("{binding} = {py_module}.{name}"));
                    }
                }
            }
            _ => self.push_source_without_macro_lines(stmt.range()),
        }
    }

    fn push_source_without_macro_lines(&mut self, range: TextRange) {
        let source = self.source_for(range).trim_end_matches('\n');
...
fn collect_codon_python_define(source: &str) -> Option<String> {
    source.lines().find_map(|line| {
        let trimmed = line.trim();
        let rest = trimmed.strip_prefix("#%define CODON_PYTHON")?.trim();
        if rest.is_empty() {
            None
        } else {
            Some(strip_optional_quotes(rest))
        }
    })
}
...
fn sanitize_module_name(module: &str) -> String {
    module
        .chars()
        .map(|ch| {
